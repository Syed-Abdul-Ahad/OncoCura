  ...
  const handleFile = (selectedFile) => {
    ...
    if (selectedFile) {
      if (allowedFileTypes.includes(selectedFile.type)) {
        ...
        setErrorMessage('Invalid file type. Please upload a PDF, JPEG, or PNG.');
        setFileName('');
        setFile(null);
      }
    } else {
...
>
    <div 
        className="max-w-xs sm:max-w-sm md:max-w-md w-full flex flex-col items-center justify-center mx-auto my-4 p-4 bg-white rounded-lg shadow-lg"
    >
        <div className="w-full flex flex-col items-center mb-4">
            <p className="font-poppins text-lg sm:text-xl font-semibold mt-4">Upload Report</p>
            <img src={'/upload.png'} alt="upload-icon" className="w-10 h-10 sm:w-12 sm:h-12 mb-2 mt-3" />
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col items-center w-full">
            <label className="relative">
                ...
                />
                <span 
                    className="cursor-pointer text-base sm:text-lg font-poppins text-black text-sm" 
                    onClick={() => document.querySelector('input[type=file]').click()}
                >
                    <span className="text-blue-600 text-sm">Browse</span> OR Drag and drop here
                </span>
            </label>
            {fileName && <p className="text-center mt-2 text-sm">{fileName}</p>}
            {errorMessage && <p className="text-red-500 text-center font-bold text-sm">{errorMessage}</p>}
            <p className="text-gray-600 text-xs text-center mt-2">PDF, JPEG, PNG - Max 5 MB</p>
            <button 
                type="submit" 
        ...
        {uploadStatus && <p className="text-center mt-4 text-sm">{uploadStatus}</p>}
    </div>
</div>
