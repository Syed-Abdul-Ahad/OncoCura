...
export const GeneratePlan = () => {
  ...
  const { records, getAllRecords, summary } = useGlobalContext();

  const analysisResult = records.find(
  ...
  return (
    <div className=" h-[200vh]  min-h-screen flex flex-col mt-2 mb-10 items-center">
      ...
      <div className="bg-white font-poppins rounded-3xl shadow-2xl border-gray-800 w-full  max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-3xl xl:max-w-5xl mx-auto p-8 text-left relative sm:-bottom-52 md:-bottom-60 lg:-bottom-72 left-5 ">
        ...
        <div className="flex justify-start mb-2">
          ...
          >
            {loading ? (
              ...
              <>
                <Link to={`/kanban-board/${params?.id}`}>
                  View Treatment Plan
                </Link>
                <img
                  src={"/chevron.png"}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="rounded-lg w-11/12 sm:w-[500px] md:w-[400px] lg:w-[350px] h-auto p-5 shadow-lg relative">
            ...
            <UploadReport setModalOpen={setModalOpen} />
          </div>
        </div>
      )}
    </div>
